...
crosspost_notification() {
	...
	if [[ -z "${input_payload}" ]]; then
		echo "crosspost_notification:: input_payload is required" >&2
		return 1
	fi
	...
	if ! command -v jq >/dev/null 2>&1; then
		echo "crosspost_notification:: jq is required but was not found in PATH" >&2
		return 1
	fi
	...
	if [[ -z "${crosspost}" ]] || [[ "${crosspost}" == "{}" ]]; then
		echo "crosspost_notification:: crosspost is empty, skipping." >&2
		return 0
	fi
	...
	echo "crosspost_notification:: sending to ${channel_count} channel(s)" >&2
