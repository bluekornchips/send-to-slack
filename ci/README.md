...
Builds the Docker image and sends a test notification to Slack.
...
```bash
CHANNEL=#test SLACK_BOT_USER_OAUTH_TOKEN=token ./ci/build-tests.sh
```

Build and test with custom tag:

```bash
CHANNEL=#test SLACK_BOT_USER_OAUTH_TOKEN=token ./ci/build-tests.sh --tag v1.0.0
```

Test using an existing image by ID:

```bash
CHANNEL=#test SLACK_BOT_USER_OAUTH_TOKEN=token ./ci/build-tests.sh --image efa4fc963b0e
```

Test using a full image reference:

```bash
CHANNEL=#test SLACK_BOT_USER_OAUTH_TOKEN=token ./ci/build-tests.sh --tag registry/repo:tag
```
...
- Variables are substituted at runtime by `build-tests.sh`
...
This file is automatically processed by `build-tests.sh` when sending test notifications. The template is expanded with environment variables before being sent to Slack.
